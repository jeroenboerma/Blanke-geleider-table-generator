...
def Component():
    ...
    Core = "Solid"          #Choose "Solid", "Hollow"
    Location = "Outside"     #Choose "Inside" or "Outside"
    Painted = "No"          #Choose "Yes" of "No"
    return Material, Shape, Core, Location, Painted

def dimensions():
    #Copy the filepath of the excel file you want to import from below
    location = r'C:\Users\AL27397\OneDrive - Alliander NV\Documents\Ketenbelastbaarheidstudie\Blanke geleiders\Handleidingen'
    filename = 'Tabellen verhoogde belastbaarheid afmetingen.xlsx'
    #Lines below import via import function, fill in sheet name and row numbers(start and end) you want to import
    Width = Import_from_excel.import_col(location, filename, 'Rechthoekig', 'A', 6, 20)/1000
    Thickness = Import_from_excel.import_col(location, filename, 'Rechthoekig', 'B', 6, 20)/1000
    Diameter = Import_from_excel.import_col(location, filename, 'Cilindrisch', 'A', 60, 107)/1000
    Wall_Thickness = Import_from_excel.import_col(location, filename, 'Cilindrisch', 'B', 60, 107)/1000
    return Width, Thickness, Diameter, Wall_Thickness
...
def select_material(Material, Shape, Painted, Location):
    ...
    #Material properties
        ...
        if Painted == "No" and Location == "Inside":
            epsilon = 0.4           #Emissivity from DIN 43671  
        elif Painted == "No" and Location == "Outside":
                epsilon = 0.6
        elif Painted == "Yes":                
            epsilon = 0.9
...
def component_parameters(Shape, Core, rho0, epsilon, Dh, w, Diameter, Wall_Thickness):
    #Component properties
    L = 2                        #Length of conductor                            [m]
    if Shape == "Cylindrical":
        if Core == "Solid":
            #Dh = 30*(1/1000)      #Outer diameter                                 [m]
            ID = 0                #Inner diameter
            B = 2*np.pi*(Diameter/2)    #Circumference                                  [m]
            S = np.pi*(Diameter/2)**2   #Cross section                                  [m2]
            w = 0                 #Width not applicable
            
        elif Core == "Hollow":
            #Dh = 27.7*2*(1/1000)        #Outer diameter
            #WD = 5*(1/1000)             #Wall thickness                                [m]
            ID = Diameter - 2*Wall_Thickness              #Inner diameter                                 [m]
            B = 2*np.pi*(Diameter/2)          #Circumference                                  [m]
            S = (np.pi/4)*(Diameter**2-ID**2) #Cross section                                  [m2]
            w = 0                       #Width not applicable
        
    elif Shape == "Rectangular":
        #Dh = 80*(1/1000)          #Height                                         [m]
        ID = 0                     #Inner diameter
        #w = 10*(1/1000)            #Width                                          [m]  
        ...
        SN = 61                 #Number of strands
        Dw = 3.23*(1/1000)       #Wire diameter  [m]
        Dh = 29.1*(1/1000)        #Outer diameter [m]
    ...
    if Shape == "Cylindrical":
        Sr = A*((1/2)*np.sqrt(2))/np.pi         #Receptive surface is 2R*L
...
def Wheather(Material, Painted):
    #Weather conditions, specified as in ABB pocket book
    if Material == "Copper" and Painted == "No":
        phis = 450         #Solar flux density                                 [W/m2]
    elif Material == "Aluminium" or Material == "Al. alloy" and Painted == "No":
        phis = 350 
    elif Painted == "Yes":
        phis = 700
    ...
    elif Material == "Aluminium" or Material == "Al. alloy":
